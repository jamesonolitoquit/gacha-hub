...
export default async function HomePage() {
  ...
  return (
    <div className="fantasy-shell px-6 py-12 text-foreground">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8">
            ...
            <aside className="fantasy-card grid min-w-0 gap-4 rounded-[1.5rem] p-5">
              <div className="min-w-0">
                ...
                <div className="mt-3 overflow-hidden rounded-[1.25rem] border border-white/10 bg-slate-950/40">
                  <div
                    className="min-h-40 bg-cover bg-center rounded-[1.25rem]"
                    style={{
        ...
        <section className="fantasy-panel w-full rounded-[2rem] p-8">
          ...
          <div id="game-rail" tabIndex={0} className="mt-6 -mx-5 px-5 pr-14 overflow-x-auto motion-scroll-rail snap-x snap-mandatory flex gap-4 py-4 relative">
            {games.map((game) => (
              <Link
                key={game.slug}
              ...
              >
                <div
                  className="absolute inset-0 w-full h-full bg-cover bg-center rounded-2xl opacity-18"
                  style={{ backgroundImage: `url(${getOptimizedBannerUrl(game.bannerUrl, 640)})` }}
                  aria-hidden="true"
                />
                <div
                  className="absolute inset-0 w-full h-full bg-gradient-to-b from-slate-950/18 via-slate-950/30 to-slate-950/60"
